...
@bot.on_message(filters.command("prefix") & filters.private)
async def setprefix_(_, message):
    babi = []
    for x in ubot._ubot:
            babi.append(x.me.id)
    ajg = await get_ultraprem()
    xx = message.from_user.id
    kntl = await message.reply("`Processing...`")
    if xx not in babi:
        await kntl.edit("Lu Siapa Anjeng ?")
        return
    if xx not in ajg:
        await kntl.edit("Maaf anda bukan pengguna Ultra Premium.")
        return

    if len(message.command) < 2:
        return await kntl.edit(f"Prefix harus berupa trigger.")
    else:
        if message.command[1].lower() == "none":
            prefix = [""]
        else:
            prefix = message.command[1:]
        try:
            x.set_prefix(x.me.id, prefix)
            await set_pref(x.me.id, prefix)
            return await kntl.edit(
                f"✅ Prefix diatur ke : {' '.join(message.command[1:])}")
        except Exception as error:
            await kntl.edit(error)
