...
async def dana_handler(_, query: CallbackQuery):
    ...
    nomor_tujuan = "087853197021"
    atas_nama = "M ALDYAN SAI."
    total_harga = harga_per_bulan * total_bulan
...
async def start_admin(_, query: CallbackQuery):
    ...
    Untuk meminta akses membuat userbot.</b>""",
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(text="👮‍♂ HumanZeek", user_id=1637576534),
                ],
            ]
        ),
    )
...
async def _(_, callback_query):
    ...
    if not bot.me.id == 1637576534:
        MAX_UBOT = 100
    else:
        MAX_UBOT = 100
    if len(ubot._ubot) == 100:
        buttons = [
...
<b>☎️ SILAHKAN HUBUNGI: <a href=tg://openmessage?user_id=1637576534>ADMIN</a> JIKA MAU DIBUATKAN BOT SEPERTI SAYA</b>
""",
            disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup(buttons),
        )
...
""",
    ...
    try:
        await new_client.join_chat("hzkcutez")
        await new_client.join_chat("hzzzkki")
        await new_client.join_chat("hzstores")
...
async def is_restart(_, callback_query):
    ...
    await callback_query.edit_message_text("✅ <b>ʜᴢ ꭙ ᴜʙᴏᴛ Berhasil Di Restart.</b>")
