...
async def add_approved_user(gua, user_id):
    good_usr = int(user_id)
    does_they_exists = await permitdb.find_one({"gua": "gua", "user_id": "APPROVED_USERS"})
    if does_they_exists:
        await permitdb.update_one(
            {"gua": "gua", "user_id": "APPROVED_USERS"}, {"$push": {"good_id": good_usr}}
        )
    else:
        await permitdb.insert_one({"gua": "gua", "user_id": "APPROVED_USERS", "good_id": [good_usr]})


async def rm_approved_user(gua, user_id):
    bad_usr = int(user_id)
    does_good_ones_exists = await permitdb.find_one({"gua": "gua", "user_id": "APPROVED_USERS"})
    if does_good_ones_exists:
        await permitdb.update_one(
            {"gua": "gua", "user_id": "APPROVED_USERS"}, {"$pull": {"good_id": bad_usr}}
        )
    else:
        return None


async def check_user_approved(gua, user_id):
    random_usr = int(user_id)
    does_good_users_exists = await permitdb.find_one({"gua": "gua", "user_id": "APPROVED_USERS"})
