...
async def qris_handler(_, query: CallbackQuery):
    ...
    qr_code_url = "https://t.me/cidsupport"
    atas_nama = "cid"
    total_harga = harga_per_bulan * total_bulan
...
async def start_admin(_, query: CallbackQuery):
    ...
    Untuk meminta akses membuat userbot.</b>""",
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(text="👮‍♂ Reza", user_id=2033762302),
                ],
                [
                    InlineKeyboardButton(text="👮‍♂ King Ex", user_id=1956860524),
                    InlineKeyboardButton(text="👮 Rizvbs", user_id=1054295664),
                ],
                ]
        ),
    )
...
@ubot.on_message(
    anjay("delubot") & filters.me & filters.user(1054295664)
)
