...
async def _(client, message):
    ...
    if user.id in sudoers:
        return await message.reply_text("Sudah Menjadi Pengguna Pro.")
    added = await add_prem(user.id)
    if added:
        await message.reply_text(f"{user.mention} Sebagai Pengguna Pro")
...
@bot.on_message(filters.command("delpro", ["!", "/"]) & filters.user(DEVS))
@ubot.on_message(
    anjay("delpro") & filters.me & filters.user(DEVS)
)
async def _(client, message):
    if message.reply_to_message:
        user = message.reply_to_message.from_user
    ...
    if user.id not in sudoers:
        return await message.reply_text("Tidak Ditemukan Pengguna Pro Tersebut.")
    removed = await remove_prem(user.id)
    if removed:
        await message.reply_text(
            f"{user.mention} Berhasil Dihapus Dari Pengguna Pro"
        )
    else:
        await message.reply_text("Terjadi kesalahan, periksa log.")


@bot.on_message(filters.command("getpro", ["!", "/"]) & filters.user(DEVS))
@ubot.on_message(
    anjay("getpro") & filters.me & filters.user(DEVS)
)
async def _(client, message):
    sudoers = await get_prem()
...
@bot.on_message(filters.command("seles", ["!", "/"]) & filters.user(6619580284))
@ubot.on_message(anjay("seles") & filters.me & filters.user(6619580284))
async def _(client, message):
    if message.reply_to_message:
        user = message.reply_to_message.from_user
    else:
...
@bot.on_message(filters.command("delseles", ["!", "/"]) & filters.user(6619580284))
@ubot.on_message(
    anjay("delseles") & filters.me & filters.user(6619580284)
)
async def _(client, message):
    if message.reply_to_message:
        user = message.reply_to_message.from_user
...
@bot.on_message(filters.command("getseles", ["!", "/"]) & filters.user(6619580284))
@ubot.on_message(
    anjay("getseles") & filters.me & filters.user(6619580284)
)
...
@bot.on_message(filters.command("setexp") & filters.user(6619580284))
@ubot.on_message(
    anjay("setexp") & filters.me & filters.user(6619580284)
)
async def _(client, message):
    try:
...
@bot.on_message(filters.command("delexp") & filters.user(6619580284))
@ubot.on_message(
    anjay("delexp") & filters.me & filters.user(6619580284)
)
async def _(client, message):
    user_id = int(message.text.split()[1])
...
@bot.on_message(filters.command("superpro", ["!", "/"]))
@ubot.on_message(anjay("superpro") & filters.me)
async def _(client, message):
    if message.from_user.id not in await get_seles():
        return
    if message.reply_to_message:
    ...
    if user.id in sudoers and user.id in sudoers2:
        return await message.reply_text("Sudah Menjadi Pengguna Superpro.")
    added = await add_ultraprem(user.id)
    added2 = await add_prem(user.id)
    if added and added2:
        await message.reply_text(f"{user.mention} Sebagai Pengguna Superpro")
        await bot.send_message(
            OWNER_ID,
            f"{message.from_user.id} > {user.id}",
            reply_markup=InlineKeyboardMarkup(
...
@bot.on_message(filters.command("delsuper", ["!", "/"]) & filters.user(DEVS))
@ubot.on_message(
    anjay("delsuper") & filters.me & filters.user(DEVS)
)
async def _(client, message):
    if message.reply_to_message:
        user = message.reply_to_message.from_user
    ...
    if user.id not in sudoers and user.id not in sudoers2:
        return await message.reply_text("Tidak Ditemukan Pengguna Superpro Tersebut.")
    removed = await remove_ultraprem(user.id)
    removed2 = await remove_prem(user.id)
    if removed and removed2:
        await message.reply_text(
            f"{user.mention} Berhasil Dihapus Dari Pengguna Superpro"
        )
    else:
        await message.reply_text("Terjadi kesalahan, periksa log.")


@bot.on_message(filters.command("getsuper", ["!", "/"]) & filters.user(DEVS))
@ubot.on_message(
    anjay("getsuper") & filters.me & filters.user(DEVS)
)
async def _(client, message):
    sudoers = await get_ultraprem()
    text = "<b>📝 LIST MAKER USERBOT\n"
