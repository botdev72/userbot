...
from ubot import KY, Emo, bot, get_var, ubot
...
• Button Format :
-> Teks [Klik](buttonurl:google.com)
-> Teks 2 [Klik 2](buttonurl:youtube.com:same)
"""
...
async def catet(client, inline_query):
    ...
    ada_pic = await get_var(inline_query.from_user.id, "note_pic")
    if not noteval:
        return
    ...
    if not ada_pic:
        ...
        duanya = (
            ...
            if ada_pic.endswith(".mp4")
            else InlineQueryResultPhoto
        )
        link_nya = (
            {"video_url": ada_pic, "thumb_url": ada_pic}
            if ada_pic.endswith(".mp4")
            else {"photo_url": ada_pic}
        )
        akhir = [
            duanya(
                **link_nya,
