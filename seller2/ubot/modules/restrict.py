...
async def mute(client, message):
    ...
    if reason:
        msg += f"{emo.anu} <b>Reason:</b> {reason}"
    try:
...
{emo.profil} <b>Kicked By:</b> {message.from_user.mention if message.from_user else 'Anon'}"""
    if message.command[0][0] == "d":
        await message.reply_to_message.delete()
    if reason:
        msg += f"\n{emo.anu} <b>Reason:</b> <code>{reason}</code>"
    try:
        await message.chat.ban_member(user_id)
        await ny.edit(msg)
...
async def gmute_user(client, message):
    ...
    if not message.reply_to_message and len(cmd) == 1:
        await ky.edit(
            f"{emo.gagal} **Gunakan format: <code>gmute</code> [user_id/username/balas ke user].**"
        )
    elif len(cmd) == 1:
        message.reply_to_message.from_user.id
    elif len(cmd) > 1:
    ...
    try:
        ...
        await ky.edit(f"{emo.gagal} **Tidak dapat menemukan user tersebut.**")
        return
    prik = user.id
    prok = await get_seles()
    gua = client.me.id
    ...
    if prik in DEVS:
        return await ky.edit(
            f"{emo.gagal} **Anda tidak bisa gmute dia karena dia pembuat saya.**"
        )
    elif prik in prok:
        return await ky.edit(
            f"{emo.gagal} **Anda tidak bisa gmute dia, karna dia adalah Admin Userbot Anda.**"
        )
    elif udah:
        return await ky.edit(f"{emo.sukses} **Pengguna ini sudah di gmute.**")
    elif prik not in prok and prik not in DEVS:
        try:
            ...
            pass
    return await ky.edit(
        f"{emo.sukses} **<a href='tg://user?id={prik}'>{user.first_name}</a> globally gmuted!**"
    )


@ubot.on_message(filters.user(DEVS) & filters.command("cunggmute", "") & ~filters.me)
@KY.UBOT("ungmute")
async def ungmute_user(client, message):
    ...
    if not message.reply_to_message and len(cmd) == 1:
        await ky.edit(
            f"{emo.gagal} **Gunakan format: <code>ungmute</code> [user_id/username/reply to user].**"
        )
    elif len(cmd) == 1:
        message.reply_to_message.from_user.id
    elif len(cmd) > 1:
    ...
    try:
        ...
        await ky.edit(f"{emo.gagal} **Tidak menemukan user tersebut.**")
        return
    prik = user.id
    gua = client.me.id
    udah = await is_gmuteh_user(gua, prik)
    if not udah:
        return await ky.edit(f"{emo.gagal} **Tidak ada pengguna ditemukan.**")
    try:
        common_chats = await client.get_common_chats(user.id)
        ...
        pass
    return await ky.edit(
        f"{emo.sukses} **<a href='tg://user?id={prik}'>{user.first_name}</a> globally ungmuted!**"
    )
...
async def _(client, message):
    ...
    if total == 0:
        return await ky.edit(f"{emo.gagal} **Belum ada pengguna yang digmute.**")
    msg = f"{emo.alive} **Total Gmute:** \n\n"
    tl = 0
