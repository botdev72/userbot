...
async def user(client, message):
    ...
    for X in ubot._ubot:
        try:
            ...
            count += 1
            # Menggabungkan informasi pengguna userbot ke dalam variabel 'user'
            user += f"""
❏ USERBOT KE {count}
...
"""
    ...
    if int(len(str(user))) > 4096:
        # Jika panjang 'user' melebihi 4096 karakter, kirim sebagai dokumen
        ...
        # Jika tidak, kirim sebagai pesan teks
        ...
        if int(_ubot["name"]) == int(callback_query.data.split()[1]):
            ...
            try:
                ...
                await callback_query.edit_message_text(
                    "✅ <b>AmangUbot Berhasil Di Restart.</b>"
                )
...
async def restart(_, message):
    ...
    await bot.send_message(
        message.chat.id,
        f"<b>Apakah kamu yakin ingin Me Restart Bot ?</b>",
        disable_web_page_preview=True,
        reply_markup=InlineKeyboardMarkup(buttons),
    )
