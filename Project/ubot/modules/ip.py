...
from ubot.utils import http


@KY.UBOT("ip", sudo=True)
async def ip_lookup(client, message):
    emo = Emo(client.me.id)
    await emo.initialize()
    if len(message.command) < 2:
        return await message.reply_text(
            f"{emo.gagal} **Berikan saya ip address untuk dicek.**"
        )
    ip_address = message.command[1]
    msg = await message.reply_text(f"{emo.proses} **Processing...**")
...
async def whois_domain_target(client, message):
    ...
    ran = await message.reply(f"{emo.proses} <b>Processing...</b>")
