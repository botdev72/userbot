...
async def _(client, message):
    ...
    if not user_id:
        return await Tm.edit(
            f"{emo.gagal} <b>Berikan userid/username/reply untuk mendapatkan info pengguna tersebut.</b>"
        )
    try:
    ...
    try:
        if len(message.text.split()) > 1:
            ...
            if message.chat.type == ChatType.PRIVATE:
                return await Tm.edit(
                    f"{emo.gagal} **Gunakan perintah ini di dalam grup atau gunakan {message.text} [group username atau id]`**"
                )
            else:
