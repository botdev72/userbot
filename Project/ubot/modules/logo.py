...
async def logo_gen(client, message):
    ...
    if not name:
        await xx.edit(f"{emo.gagal} <b>Contoh :</b><code>{message.text}</code> [text]")
        return
    bg_, font_ = "", ""
    ...
    flnme = f"logo.png"
    img.save(flnme, "png")
    await xx.edit(f"{emo.proses} <b>Uploading</b>")
    if os.path.exists(flnme):
        await client.send_photo(
            chat_id=message.chat.id,
            photo=flnme,
            caption=f"{emo.sukses} <b>Writed by :{client.me.mention}</b>",
        )
        os.remove(flnme)
        await xx.delete()
