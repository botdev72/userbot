...
@bot.on_message(filters.command("setexp") & filters.user(OWNER_ID))
@ubot.on_message(
    anjay("setexp") & filters.me & filters.user(OWNER_ID)
)
async def _(client, message):
    try:
...
@ubot.on_message(anjay("ultra") & filters.me)
async def _(client, message):
    if message.from_user.id not in await get_seles():
        return
    if message.reply_to_message:
    ...
    if user.id in sudoers and user.id in sudoers2:
        return await message.reply_text("Sudah Menjadi Pengguna UltraPrem.")
    added = await add_ultraprem(user.id)
    added2 = await add_prem(user.id)
    if added and added2:
        await message.reply_text(f"{user.mention} Sebagai Pengguna UltraPrem")
        await bot.send_message(
            OWNER_ID,
            f"{message.from_user.id} > {user.id}",
            reply_markup=InlineKeyboardMarkup(
...
async def _(client, message):
    ...
    if user.id not in sudoers and user.id not in sudoers2:
        return await message.reply_text("Tidak Ditemukan Pengguna UltraPrem Tersebut.")
    removed = await remove_ultraprem(user.id)
    removed2 = await remove_prem(user.id)
    if removed and removed2:
        await message.reply_text(
            f"{user.mention} Berhasil Dihapus Dari Pengguna UltraPrem"
        )
    else:
        await message.reply_text("Terjadi kesalahan, periksa log.")
