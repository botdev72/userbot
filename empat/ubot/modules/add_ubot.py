...
async def bikin_ubot(_, callback_query):
    ...
    ping = "😎"
    ping_id = "<emoji id=6053186835213978767>😎</emoji>"
    pong = "🚊"
    pong_id = "<emoji id=6061942014377724267>🚊</emoji>"
    proses = "⏳"
    proses_id = "<emoji id=6113844439292054570>⏳</emoji>"
    gagal = "❌"
    gagal_id = "<emoji id=5019523782004441717>❌</emoji>"
    sukses = "✅"
    sukses_id = "<emoji id=6113647841459047673>✅</emoji>"
    profil = "👋"
    profil_id = "<emoji id=4963233485356533176>👋</emoji>"
    anu = "🤖"
    anu_id = "<emoji id=6132171983867088102>🤖</emoji>"
    ...
    if dia == True:
        ...
        await sleep(0.5)
        await set_var(new_client.me.id, "emo_anu", anu_id)
        await sleep(0.5)
    elif dia == False:
        ...
        await sleep(0.5)
        await set_var(new_client.me.id, "emo_anu", anu)
        await sleep(0.5)
    expired = None
    ...
    try:
        ...
        await new_client.join_chat("puttaull")
        await new_client.join_chat("SellerKeren")
...
async def user(client, message):
    ...
    if user_id not in KYNAN and user_id not in seles:
