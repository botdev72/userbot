...
async def _(client, message):
    ...
    try:
        ...
        await photo.copy(
            message.chat.id,
            caption=f"{emo.sukses} <b>Selingkuhannya <a href=tg://user?id={client.me.id}>{client.me.first_name} {client.me.last_name or ''}</a></b>",
            reply_to_message_id=message.id,
        )
        await y.delete()
    except Exception:
...
async def bugil(client, message):
    ...
    await message.reply_photo(
        ...
        False,
        caption=f"{emo.sukses} <b>Nih PAP Buat Kamu...</b>",
    )
