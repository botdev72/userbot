...
async def _(client, message):
    ...
    if done > 0:
        await msg.edit(
            f"{emo.sukses}**Berhasil Terkirim:** `{done}` \n**{emo.gagal}Gagal Mengirim Pesan Ke:** `{failed}`."
        )
    else:
        await msg.edit(f"{emo.sukses} **Pesan Broadcast Berhasil Dibatalkan**.")
...
async def cancel_broadcast(client, message):
    ...
    if not broadcast_running:
        return await eor(
            message,
            f"{emo.gagal} <b>Tidak ada pengiriman pesan global yang sedang berlangsung.</b>",
        )

    broadcast_running = False
    # await message.reply(f"{emo.sukses} <b>Pengiriman pesan global telah dibatalkan!</b>")
...
async def _(client, message):
    ...
    if done > 0:
        await client.send_message(
            message.chat.id,
            f"{emo.sukses} **Successfully send Message To `{done}` Users chat.**"
        )
    else:
        await message.edit(f"{emo.sukses} **Pesan Broadcast Berhasil Dibatalkan**.")
