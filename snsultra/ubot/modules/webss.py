...
async def take_ss(_, message):
    if len(message.command) < 2:
        return await message.edit("<code>Berikan saya link yang valid</code>")
    ...
    if len(message.command) == 2:
        ...
        return await message.edit("<code>Ada yang salah.</code>")

    m = await message.edit("<code>Processing...</code>")
