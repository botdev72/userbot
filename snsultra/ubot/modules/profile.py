...
async def list_admin(client, message):
    bacot = await message.edit("`Processing...`")
    a_chats = []
...
async def unblock_user_func(client, message):
    ...
    tex = await message.edit("<code>Processing . . .</code>")
    if not user_id:
        return await eor(
    ...
    await message.edit(f"<b>Berhasil membuka blokir</b> {umention}")
...
async def block_user_func(client, message):
    ...
    tex = await message.edit("<code>Processing . . .</code>")
    if not user_id:
        return await message.edit("Berikan username untuk di blok.")
    if user_id == client.me.id:
