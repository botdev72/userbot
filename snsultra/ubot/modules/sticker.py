...
async def _(client, message):
    if len(message.command) < 2:
        return await message.edit("<code>memes</code> [text]")
...
async def kang(client, message):
    ...
    Tm = await message.edit("Boleh juga ni stickernya colong ahh...")
    media_ = None
    emoji_ = None
    is_anim = False
    is_video = False
...
async def packinfo(client, message):
    rep = await message.edit("Processing...")
...
async def memify(client, message):
    if not message.reply_to_message:
        await message.edit("Balas ke pesan foto atau sticker!")
        return
    reply_message = message.reply_to_message
    if not reply_message.media:
        await message.edit("Harap Balas ke foto atau sticker!")
        return
...
async def stick2png(client, message):
    try:
        Tm = await message.edit("Downloading . . .")
