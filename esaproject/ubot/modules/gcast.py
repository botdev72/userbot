...
async def cancel_broadcast(client, message):
    ...
    if not broadcast_running:
        return await message.edit("<code>Tidak ada pengiriman pesan global yang sedang berlangsung.</code>")

    broadcast_running = False
    await message.edit("<code>Pengiriman pesan global telah dibatalkan!</code>")
...
async def bl_chat(client, message):
    ...
    if chat.type == "private":
        return await message.edit("Maaf, perintah ini hanya berlaku untuk grup.")
    user_id = client.me.id
    bajingan = await blacklisted_chats(user_id)
    if chat in bajingan:
        return await message.edit("Obrolan sudah masuk daftar Blacklist Gcast.")
