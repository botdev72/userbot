...
async def _(client, message):
    y = await message.edit("<b>🔍 Mencari Video Asupan...</b>")
    try:
        asupannya = []
...
async def _(client, message):
    y = await message.edit("<b>🔍 Mencari Ayang...</b>")
    try:
        ayangnya = []
...
async def _(client, message):
    if message.chat.id in BLACKLIST_CHAT:
        return await message.edit("<b>Maaf perintah ini dilarang di sini</b>")
    y = await message.edit("<b>🔍 Mencari Video Bokep...</b>")
    try:
...
async def anim(client, message):
    iis = await message.edit("🔎 <code>Search Anime...</code>")
...
async def nimek(client, message):
    erna = await message.edit("🔎 <code>Search Anime...</code>")
    await message.reply_photo(
        choice(
            [
                tai.photo.file_id
...
async def bugil(client, message):
    kazu = await message.edit("🔎 <code>Nih PAP Nya...</code>")
    await message.reply_photo(
        choice(
            [
                lol.photo.file_id
