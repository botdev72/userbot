...
async def set_chat_photo(client: Client, message: Message):
    ...
    if not (can_change_admin or can_change_member):
        await message.edit("<code>Kamu tidak punya akses wewenang</code>")
    if message.reply_to_message:
        ...
        await message.edit("<code>Mohon balas ke media</code>")
...
async def member_unban(client: Client, message: Message):
    ...
    zz = await message.edit("<code>Processing...</code>")
...
async def pin_message(client: Client, message):
    ...
    try:
        ...
        return await message.edit("<b>Anda bukan admin di group ini !</b>")


@ubot.on_message(anjay("mute") & filters.me)
async def mute(client: Client, message: Message):
    user_id, reason = await extract_user_and_reason(message)
    nay = await message.edit("<code>Processing...</code>")
    if not user_id:
...
async def unmute(client: Client, message: Message):
    ...
    kl = await message.edit("<code>Processing...</code>")
    if not user_id:
...
async def kick_user(client: Client, message: Message):
    user_id, reason = await extract_user_and_reason(message)
    ny = await message.edit("<code>Processing...</code>")
    if not user_id:
...
async def promotte(client: Client, message: Message):
    user_id = await extract_user(message)
    biji = await message.edit("<code>Processing...</code>")
    if not user_id:
...
async def demote(client: Client, message: Message):
    user_id = await extract_user(message)
    sempak = await message.edit("<code>Processing...</code>")
    if not user_id:
...
async def _(client, message):
    ...
    if total == 0:
        return await message.edit("`Belum ada pengguna yang digban.`")
    nyet = await message.edit("`Processing...`")
    msg = "**Total Gbanned:** \n\n"
    tl = 0
