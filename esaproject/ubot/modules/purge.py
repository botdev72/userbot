...
from . import bot, ubot, anjay, eor
...
async def purge_me_func(client, message):
    ...
    if not n.isnumeric():
        return await message.edit("Argumen Tidak Valid")
    n = int(n)
    if n < 1:
        return await message.edit("Butuh nomor >=1-999")
    ...
    if not message_ids:
        return await message.edit(text="Tidak ada pesan yang ditemukan.")
    ...
    for hundred_messages_or_less in to_delete:
        ...
        mmk = await message.edit(f"✅ {n} Pesan Telah Di Hapus")
...
async def purgefunc(client, message):
    ...
    if not message.reply_to_message:
        return await message.edit("Membalas pesan untuk dibersihkan.")
    chat_id = message.chat.id
    message_ids = []
