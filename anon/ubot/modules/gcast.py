...
async def _(client, message):
    ...
    msg = await message.reply(f"{emo.proses} **Sabar Kontol Lagi Di Gikesin...**")
    ...
    if done > 0:
        return await msg.edit(
            f"{emo.alive} **Nyepong Sukses :\n{emo.sukses} Berhasil Nyepong `{done}` Grup.\n{emo.gagal} Gagal Nyepong `{failed}` Grup.**",
        )
...
async def _(client, message):
    ...
    if done > 0:
        await client.send_message(
            message.chat.id,
            f"{emo.alive} **Pesan Terkirim :\n{emo.sukses} Berhasil di `{done}` penguna.\n{emo.gagal} Gagal di `{failed}` pengguna.**",
        )
    else:
        await client.send_message(
            message.chat.id, f"{emo.sukses} **Pesan Broadcast Berhasil Dibatalkan.**"
        )
