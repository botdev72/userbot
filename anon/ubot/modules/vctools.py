...
• Penjelasan: Untuk memulai hubungan voice chat grup.

• Perintah: <code>{0}stopvc</code>
• Penjelasan: Untuk mengakhiri hubungan voice chat grup.
...
async def opengc(client, message):
    ...
    ky = await message.edit(f"{emo.proses} <b>Processing....</b>")
    await sleep(2)
...
async def end_vc_(client, message):
    ...
    await ky.edit(
        f"{emo.sukses} <b>Obrolan Suara Diakhiri dengan kesedihan</b>\n<b>• Chat :</b> <code>{message.chat.title}</code>"
    )
...
async def joinvc(client, message):
    ...
    for x in ubot._ubot:
        x.me.id
    ky = await message.edit(f"{emo.proses} <b>Otw naik....</b>")
    ...
    if chat_id:
        ...
        try:
            ...
            await sleep(2)
            await ky.edit(
                f"{emo.sukses} <b>Berhasil menjalin hubungan Voice Chat</b>\n<b>• Chat :</b> <code>{message.chat.title}</code>"
            )
            await sleep(1)
...
async def leavevc(client, message):
    ...
    for x in ubot._ubot:
        x.me.id
    ky = await message.edit(f"{emo.proses} <b>Otw turun....</b>")
    chat_id = message.command[1] if len(message.command) > 1 else message.chat.id
    with suppress(ValueError):
        chat_id = int(chat_id)
    if chat_id:
        try:
            ...
            await ky.edit(
                f"{emo.sukses} <b>Berhasil Meninggalkan Hubungan Voice Chat</b>\n• <b>Chat : </b><code>{message.chat.title}</code>"
            )
        except Exception as e:
...
async def liat_os(client, message):
    emo = Emo(client.me.id)
    await emo.initialize()
    ky = await message.edit(f"{emo.proses} **Processing...**")
    for y in ubot._ubot:
